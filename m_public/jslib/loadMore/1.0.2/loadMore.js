...
(function (w, f) {
    ...
    if (typeof define === 'function') {
        define('loadMore/1.0.2/loadMore', ['jquery', 'lazyload/1.9.1/lazyload'], function (require) {
            // jquery库
            var $ = require('jquery');
     ...
     * 数据总条数
     * total:100
     * 标识是否滚动内容是在页面中的部分模块，并非整个document范围内
     * partScroll:false
     * 滚动内容的id
     * partSrollerId:'#document'
     * 滚动容器的id
    ...
    function LoadMore(options) {
        ...
        if (isScroll) {
            ...
            /**
             * 页面部分内容滚动时绑定滚动事件，监听是否到达底部，执行加载更多操作
             */
            if(partScroll) {
                options.iscroll.on('scroll', function () {
                    // 通过iscroll监听滚动
                    var scrollTop = -this.y;
                    // 滚动内容的高度
                    var scrollH = $(options.partSrollerId).height();
                    if (loadFlag && scrollTop + $(options.partScrollBox).height() >= scrollH) {
                        // 此判断用于当第一次拖拽到底部时允许展示全部底部而不执行加载更多
                        if (firstDragFlag) {
                            firstDragFlag = false;
                        } else {
