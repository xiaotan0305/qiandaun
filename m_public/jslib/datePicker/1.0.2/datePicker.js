...
define('datePicker/1.0.2/datePicker', ['jquery', 'iscroll/2.0.0/iscroll-lite'], function (require, exports, module) {
    ...
    function DatePicker(opts) {
        this.dateBoxSelector = opts.dateBoxSelector || '.dateBox';
        this.maskSelector = opts.maskSelector || '.layer';
        this.dateTxtArrSelector = opts.dateTxtArrSelector || '.dateTxt span';
        this.yearTxtArrSelector = opts.yearTxtArrSelector || '.year';
        this.confirmBtnSelector = opts.confirmBtnSelector || '#confirm';
        this.closeBtnSelector = opts.closeBtnSelector || '#close';
    ...
    DatePicker.prototype = {
        ...
        init: function () {
            var that = this;
            var dateBox = $(that.dateBoxSelector);
            that.dateBox = dateBox;

            that.mask = $(that.maskSelector);
            that.dateTxtArr = $(that.dateTxtArrSelector);
            that.yearTxtArr = $(that.yearTxtArrSelector);

            that.confirmBtn = dateBox.find(that.confirmBtnSelector);
            that.closeButton = dateBox.find(that.closeBtnSelector);

            var todayDate = new Date(Number(today));
            // 这里直接获取后两位
            // that.nowYear = Number(todayDate.getFullYear().toString().substr(-2));
            that.nowYear = todayDate.getFullYear() + '';
            that.nowMonth = todayDate.getMonth() + 1 + '';
            that.nowDay = todayDate.getDate() + '';
            ...
            that.dateBox.on('touchmove', function(e) {
                e.preventDefault();
                e.stopPropagation();
                console.log('box touchmove');
            });

            that.mask.on('touchmove', function(e) {
                e.stopPropagation();
                e.preventDefault();
            ...
            that.confirmBtn.on('click', function () {
                ...
                var bd = y + that.delimiter + m + that.delimiter + d,
                    ed = y1 + that.delimiter + m1 + that.delimiter + d1;
                that.yearTxtArr.eq(0).html('20' + y + '年');
                // 设置开始日期的显示及设置传入后台需要的值
                dateTxt.attr('data-date', bd).html(m + '月' + d + '日');
                that.yearTxtArr.eq(1).html('20' + y1 + '年');
                // 设置结束日期的显示及设置传入后台需要的值
                dateTxt1.attr('data-date', ed).html(m1 + '月' + d1 + '日');
                // 隐藏弹窗
                that.hide();
                // 如果存在确认函数的话，执行确认函数
                if (that.confirmFunc) {
                    that.confirmFunc('20' + bd, '20' + ed);
                }
            });
            ...
            that.mask.show();
            // 获取所有的选择器ul的父节点
            that.ulArr = that.dateBox.find('.blue_dateCon');
            // 获取选择器的个数
            ...
            for (var i = 0; i < that.ulArrL; i++) {
                var isl = new iScrollLite(that.ulArr[i], {
                    // 滚动为该节点
                    bindToWrapper: true,
                    // 不可横向滚动
                    scrollX: false,
                 ...
                 */
                isl.on('scrollEnd', function () {
                    that.setPosition(this);
                });
                // 将选择器实例储存为数组索引
                that.iScrollLiteArr.push(isl);
            }
        },
        ...
        setPositionByDate: function (start, end) {
            var that = this,
                ...
                idx = -1;
            // 获取传入结束日期月份的总天数
            var startDay = new Date(startArr[0], startArr[1], 0).getDate() + '';
            var endDay = new Date(endArr[0], endArr[1], 0).getDate() + '';
            ...
            if (endArr[0] > that.nowYear
                || endArr[0] === that.nowYear && endArr[1] > that.nowMonth
                || endArr[0] === that.nowYear && endArr[1] === that.nowMonth && endArr[2] > that.nowDay) {
            ...
            if (startArr[0] > endArr[0]
                ...
                || startArr[0] === endArr[0] && startArr[1] === endArr[1] && startArr[2] > endArr[2]) {
                startArr[0] = endArr[0];
                startArr[1] = endArr[1];
                startArr[2] = endArr[2];
                startDay = startArr[2];
            }
            if (startArr[2] > startDay) {
                startArr[2] = startDay;
            }
            // 开始日期选择器中的年份选择重置显示，大于结束时间年份的都隐藏
            that.ulArr.eq(0).find('ul').children().each(function () {
                var $this = $(this);
            ...
            if (startArr[0] === endArr[0]) {
                ...
                that.ulArr.eq(1).find('ul').children().each(function () {
                    var $this = $(this);
                    var day = Number($this.html());
                    if (day && day > endArr[1]) {
                        $this.hide();
                ...
                if (startArr[1] === endArr[1]) {
                    ...
                    that.ulArr.eq(2).find('ul').children().each(function () {
                        var $this = $(this);
                        var day = Number($this.html());
                        if (day && day > endArr[2]) {
                            $this.hide();
                ...
                // 由于年份不等，月份全部显示
                that.ulArr.eq(1).find('ul').children().show();
                // 天数只有大于本月天数隐藏即可
                that.ulArr.eq(2).find('ul').children().each(function () {
                    var $this = $(this);
                    var day = Number($this.html());
            ...
            that.ulArr.eq(3).find('ul').children().each(function () {
                var $this = $(this);
                var day = Number($this.html());
            ...
            if (endArr[0] === that.nowYear) {
                ...
                that.ulArr.eq(4).find('ul').children().each(function () {
                    var $this = $(this);
                    var day = Number($this.html());
                ...
                if (endArr[1] === that.nowMonth) {
                    that.ulArr.eq(5).find('ul').children().each(function () {
                        var $this = $(this);
                        var day = Number($this.html());
                ...
                that.ulArr.eq(4).find('ul').children().show();
                that.ulArr.eq(5).find('ul').children().each(function () {
                    var $this = $(this);
                    var day = Number($this.html());
                    if (day && day > endDay) {
            ...
            for (; i < that.ulArrL; i++) {
                ...
                ul.attr('data-val', r);
                that.iScrollLiteArr[i].refresh();
                that.iScrollLiteArr[i].scrollTo(0, -(idx - 3) * that.singleLiHeight);
            }
        }
    };
