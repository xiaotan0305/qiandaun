      ...
      function recalHeaderPosition(table) {
        ...
        table.find('thead tr').each(function() {
          $(this).find('th').each(function(i) {
            ...
            if($(this).prev().css('left')) {
              var leftOffset = Number($(this).prev().css('left').replace('px', '')) + $(this).prev().width();
              $(this).css('left', leftOffset + 4 +'px');
            }
          });
        });
        var firstWidth = table.find('tbody tr:first td:first a').width();
        table.find('tbody tr').each(function() {
          $(this).find('td').each(function(j) {
            if(j === 0) {
              if (firstWidth > showWidth) {
                $(this).css('width',firstWidth + 'px');
              } else {
                $(this).find('p').css('width',widthArr[j] + 4 + 'px');
              }
            } else {
              $(this).find('p').css('width',widthArr[j] + 4 + 'px');
            }
        ...
        table.find('thead tr').each(function(i) {
          if(i < getHeadRowCount(table)) {
            $(this).find('th').each(function(j) {
              $(this).css('left', leftPosition[j] +'px');
            });
          }
        });
      }
