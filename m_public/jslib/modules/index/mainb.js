 ...
 */
define('modules/index/mainb', ['jquery', 'util/util', 'modules/index/locate', 'swipe/3.3.1/swiper',
    ...
    function (require) {
        ...
        $(document.body).on('click', 'a', function (e) {
            ...
            if (href && !/javascript:/.test(href)) {
                if (/(\?|\&)u=/.test(href)) {
                    if (/u=.+\?/.test(href)) {
                        location.href = href + '&f=2';
                    } else {
                        location.href = href + '?f=2';
                    }
                } else {
                    location.href = href + (/&|\?/.test(href) ? '&' : '?') + 'f=2';
                }
            }
        });
        // 回到顶部
        ...
        //UC浏览器 touchmove 会导致渲染问题
        var events = /ucbrowser/.test(UA) ? 'scroll' : 'scroll touchmove';
        $window.on(events, function (ev) {
        ...
        $news.on('click', 'a', function () {
            ...
            // 拼接统计代码地址
            var url = '/data.d?m=adtj&city=' + encodeURIComponent(encodeURIComponent(vars.zhcity)) + '&url=' + pageUrl + '&f=2';
            $.get(url, function () {
        ...
        function indexTongJi(urlOrOther, type, channel, order, housetype, url) {
            ...
            $.get('/main.d?m=indexTj', {
                ...
                if (channel === 'index_activity') {
                    window.location = url + (/&|\?/.test(url) ? '&' : '?') + 'ctm=2.' + vars.mrCity + '.dsy.hjy.' + order + '&f=2';
                } else {
                    window.location = url + (/&|\?/.test(url) ? '&' : '?') + 'f=2';
                }
            });
        }
                    ...
                    $.get('/main.d?m=xiHuanLouPanList', ajaxData, function (result) {
                        ...
                        if (result) {
                            ...
                            if (behaviorBtn.length) {
                                ...
                                var index = arr.indexOf(Math.max.apply(Math, arr));
                                if (index == '2') {
                                    behaviorBtn.attr('href', vars.mainSite + 'zf/' + vars.mrCity + '/?f=2');
                                }
                                if (index == '1') {
                                    behaviorBtn.attr('href', vars.mainSite + 'esf/' + vars.mrCity + '/?jhtype=esf&f=2');
                                }
                                if (index == '0') {
                                    behaviorBtn.attr('href', vars.mainSite + 'xf/' + vars.mrCity + '/?f=2');
                                }
                            }
