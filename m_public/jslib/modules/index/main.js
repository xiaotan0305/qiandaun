...
define('modules/index/main', ['jquery', 'util/util', 'modules/index/locate', 'swipe/3.3.1/swiper',
    ...
    function (require) {
        ...
        $news.on('click', 'a', function () {
            ...
            // 拼接统计代码地址
            var url = '/data.d?m=adtj&city=' + encodeURIComponent(encodeURIComponent(vars.zhcity)) + '&url=' + pageUrl;
            $.get(url, function () {
                    ...
                    $.get('/main.d?m=xiHuanLouPanList', ajaxData, function (result) {
                        ...
                        if (result) {
                            ...
                            if (behaviorBtn.length) {
                                ...
                                if (zfHX || zfQYSQ) {
                                    behaviorBtn.attr('href', vars.mainSite + 'zf/' + vars.mrCity);
                                }
                                if (esfQYSQ || esfHX) {
                                    behaviorBtn.attr('href', vars.mainSite + 'esf/' + vars.mrCity);
                                }
                                if (xfXQ || xfWY) {
                            ...
                            /**
                             * 循环遍历猜你喜欢的a标签，增加点击统计需要的id，类名为color9跳过
                             */
                            for (var i = 0; i < listAArrL; i++) {
                                var $a = listAArr.eq(i),
                                    // 点击统计的前缀
                                    id = 'wapdsy_D04_';
                                if ($a.hasClass('color9')) {
                                    continue;
                                }
                                if (i < 10) {
                                    id += '0';
       ...
       if (vars.sfsource && vars.city === 'bj') {
            ...
            require.async('//clickm.fang.com/click/new/clickm.js', function () {
                Clickstat.eventAdd(window, 'load', function () {
                    Clickstat.batchEvent('wapdsy_', vars.city);
                });
            });
        }
        // 设置首页走势图
        ...
        if (/s=bdfj/.test(lh)) {
            setTimeout(function () {
                $('html,body').animate({scrollTop: $('#fjxq').offset().top - headerTopH}, 800);
            }, 500);
        }
            ...
            var navHistory = vars.localStorage && vars.localStorage.getItem('navHistory') || '';
            var ssarr = [];
            ...
            vars.localStorage && vars.localStorage.setItem('navHistory', navHistory);
        ...
        vars.localStorage && vars.localStorage.setItem('navHistory', navItem);
            navHistory = vars.localStorage && vars.localStorage.getItem('navHistory') || '';
	   }
			...
			var zjHref = $("[fname='"+navArr[i]+"']").attr('href');
                ...
                var $esfHjy = $('#esfHjy');
                if($esfHjy){
                    $.get('https://m.fang.com/esf/'+vars.city+'/?c=esf&a=wapHjy&imei='+globalCookie,  function (result) {
                        if (result) {
                        	var wirelessUrl = 'http:' + result[0].normalColumnData.wirelessUrl;
                        	var wirelessUrl2 = wirelessUrl;
                        	 if (wirelessUrl && !/javascript:/.test(wirelessUrl)) {
                                 if (/(\?|\&)u=/.test(wirelessUrl)) {
                                     if (/u=.+\?/.test(wirelessUrl)) {
                                    	 wirelessUrl2 += '&amp;source=waphjy2';
                                     }else {
                                    	 wirelessUrl2 += '?source=waphjy2';
                                     }
                                 }else {
                                	 wirelessUrl2 += (/&|\?/.test(wirelessUrl) ? '&amp;' : '?') + 'source=waphjy2';
                                 }
                             }
                            $esfHjy.html('<a id="wapdsy_D03_02" href="javascript:void(0)"'+
                           'data-url="'+wirelessUrl2+'"'+
                        'data-params="'+wirelessUrl+';click;index_activity;2;;'+wirelessUrl2+'">'+
                           '<dl><dt><h3>'+result[0].normalColumnData.wirelessTitle+'</h3><p>'+result[0].normalColumnData.wirelessSummary+'</p></dt>'+
							'<dd><img class="lazyload" data-original="'+result[0].normalColumnData.wirelessImg+'" alt="" src="'+result[0].normalColumnData.wirelessImg+'" style="display: inline;"></dd></dl></a>');
                        }
                    });
