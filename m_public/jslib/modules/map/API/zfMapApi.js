 ...
 * 租房地图移动，缩放，画地铁线，画点
 */
define('modules/map/API/zfMapApi', ['jquery', 'modules/map/API/BMap'], function (require) {
    'use strict';
    var $ = require('jquery');
    ...
    MapApi.prototype = {
        ...
        clickMarker: function (info, obj) {
            ...
            if (info.type === 'loupan') {
                ...
                if (vars.localStorage) {
                    var p = vars.localStorage.getItem('visitedZfProj');
                    if (!p) {
                        vars.localStorage.setItem('visitedZfProj', info.projcode + ';');
                    } else if (p.indexOf(info.projcode + ';') === -1) {
                        vars.localStorage.setItem('visitedZfProj', p + info.projcode + ';');
                    }
                }
        ...
        createMarker: function (info) {
            ...
            if (info.type === 'station') {
                ...
                // 大红点样式套数显示
                ltext = '<p>' + info.projname + '&nbsp;' + info.count + '</p>';
            } else {
                // 小红点样式套数隐藏
                ltext = '<p style="display:none;">' + info.projname + '&nbsp;' + info.count + '</p>';
            }
    ...
    ComplexCustomOverlay.prototype.initialize = function (map) {
        ...
        if (this._overlayMode === 'loupan') {
            ...
            // 楼盘id属性
            div.attr('id', 'wapzfditu_B01_11').attr('data-projcode', this._info.projcode);

            // 和地铁点击区分加一个class
            div.addClass('loupan');
            // 点击过的标点，变为粉色
            var p = vars.localStorage ? vars.localStorage.getItem('visitedZfProj') : null;
            if (p && p.indexOf(this._info.projcode + ';') !== -1) {
                div.addClass('visited');
            }
        } else if (this._overlayMode === 'subwayline') {
            // 地铁
            div.attr('id', 'wapzfditu_B01_18').attr('data-id', this._info.id);
