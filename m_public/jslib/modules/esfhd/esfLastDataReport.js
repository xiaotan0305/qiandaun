define('modules/esfhd/esfLastDataReport', ['jquery', 'weixin/2.0.0/weixinshare', 'superShare/1.0.1/superShare', 'rotate/esfrotate'],
    function (require, exports, module) {
        ...
        module.exports = function () {
            ...
            //控制音乐播放
            //点击跳转到公寓列表页
            $(".p12-2").on("touchstart", function() {
                $(".wrap").unbind();
                window.location = vars.esfSite + vars.encity + '/';
                return false;
            });
            var isFirst = true;
            ...
            $(".wrap").on("touchend", function(e) {
                ...
                if(target.className.indexOf('icon_m') !== -1){
                    ...
                    if (!$(".icon_m").hasClass('play')) {
                        ...
                        $(".icon_m").addClass('close');
                    } else {
                        $('#audio')[0].play();
                        $(".icon_m").removeClass('play');
                        $(".icon_m").removeClass('close');
                    }
                    return false;
                }
                touchEnd(e, $(this));
            ...
            function touchEnd(e, that) {
                var moveEndY = e.originalEvent.changedTouches[0].pageY,
                    Y = moveEndY - startY;
                if ( Y > 0) {
                    if (that.attr('id') !== 'page0') {
                        that.hide();
                        that.prev().show();
                    }
                } else if ( Y < 0) {
                    if (that.attr('id') !== 'page12') {
                        that.hide();
                        that.next().show();
                    }
                }
