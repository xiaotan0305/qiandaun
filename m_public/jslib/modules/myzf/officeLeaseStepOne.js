 ...
 * by lina 20161215
 */
define('modules/myzf/officeLeaseStepOne', ['verifycode/1.0.0/verifycode', 'slideFilterBox/1.0.0/slideFilterBox'],
        ...
        module.exports = function () {
            ...
            $.fn.countTo = function (options) {
                ...
                return $(this).each(function () {
                    ...
                    var ele = this,
                        ...
                        loopCount = 0,
                        value = settings.from,
                        data = $ele.data('countTo') || {};

                    function render(value) {
                        var formattedValue = settings.formatter.call(ele, value, settings);
                    ...
                    function updateTimer() {
                        ...
                        if (typeof(settings.onUpdate) == 'function') {
                            settings.onUpdate.call(ele, value);
                        }

                        if (loopCount >= loops) {
                            // remove the interval
                            $ele.removeData('countTo');
                            clearInterval(data.interval);
                            value = settings.to;

                            if (typeof(settings.onComplete) == 'function') {
                                settings.onComplete.call(ele, value);
                            }
                        }
                    }

                    $ele.data('countTo', data);
            ...
            function submitVerification() {
                ...
                if (!$('#messageCodeDl').is(':hidden')) {
                    if (dContent.is(':hidden') && addressManualVal !== '' && contactpersonManualVal !== '' && mobilecodeManualVal !== ''
                        && messagecodeManualVal !== '' && districtManualVal !== '请选择区域' && comareaManualVal !== '请选择商圈') {
                    ...
                    if (dContent.is(':hidden') && addressManualVal !== '' && contactpersonManualVal !== '' && mobilecodeManualVal !== ''
                        && districtManualVal !== '请选择区域' && comareaManualVal !== '请选择商圈') {
                        $btnPay.removeClass('noClick');
                    } else {
                        $btnPay.addClass('noClick');
                    }
            ...
            $btnPay.on('click', function () {
                ...
                var verifySuccess = function () {
                    ...
                    // 成功后跳转地址
                    var sucurl = vars.mySite + '?c=myzf&a=officeLeaseTwo&city=' + vars.city + '&pubtype=' + vars.pubtype + '&edit=' + vars.edit
                        + '&oldhouseid=' + vars.oldhouseid;
                    // 验证手机号
                ...
                if (parseInt(vars.authenticated) === 1 && parseInt(vars.housecount) !== 0) {
                    // 已经登录了,不用再验证验证码
                    verifySuccess();
                } else {
            ...
            var $complainFloat = $('#complainFloat');
            // 点击申诉弹框的是
            $('#yes').on('click', function () {
                var mobileVal = $('#mobilecodeManual').val();
                $complainFloat.hide();
                $.ajax({
                    url: vars.mySite + '?c=myzf&a=ajaxComplain&mobile=' + mobileVal + '&city=' + vars.city,
                    success: function (data) {
