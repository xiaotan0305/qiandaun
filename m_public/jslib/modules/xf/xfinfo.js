define('modules/xf/xfinfo', [
    ...
    shoucang.on('click', function () {
        if (!userid) {
            ...
            if ($(this).hasClass('on')) {
                ...
                $.get('/xf.d?m=addFav&userphone=' + userphone + '&username=' + username + '&userid=' + userid + '&city=' + vars.paramcity + '&newcode=' + vars.paramid + '&projname=' + fangName + '&image=http:'+ $('#slider img').eq(0).attr('src') + '&price=' + vars.xfinfoprice + '&address=' + vars.xfinfoaddress + '&math=' + Math.random(),
                    function (data) {
                        if (data.root.code === '100') {
    ...
    // 主力户型 和 点评 控制星星亮
    $('#huxingslider .star-s, .ico-star').each(function () {
    ...
    $.getJSON('/data.d?m=ad&type=xfinfo&city=' + zhcity, function (data) {
        ...
        if (result && result.length > 0) {
            if (document.getElementById('sfxfpo')) {
                ...
                $spanHtml += '</ul></nav>';
                $spanHtml += '<img src="//countub.3g.fang.com/countub?id='+adIds.join(",")+'" width="0" height="0" style="display: none;" >';
    ...
    function tongji(url) {
        ...
        $.ajax({url: '/data.d?m=adtj&city=' + zhcity + '&url=' + pageUrl, async: false});
        window.location = url;
    }
	...
	// 下拉显示app下载
	$('#loupan').prepend('<div class="loupanMarginTop" style="height: 44px;"></div>');
	function showApp () {
		var scrollTop = $(window).scrollTop();
		if (scrollTop >= 90) {
			$('.xf-xq').addClass('fixed').css('position', 'fixed');
			$('.loupanMarginTop').show();
		} else {
			$('.xf-xq').removeClass('fixed').css('position', 'relative');
			$('.loupanMarginTop').hide();
		}
	}
	showApp();
	...
	$('.icon-nav').on('click', function () {
		if ($('.newNav').is(':hidden')) {
			...
			$('.xf-xq').removeClass('fixed').css({'position': 'relative', 'top': 0});
		}
	});
