define('modules/xf/xfinfo_seo2', [
    ...
    // 收藏-----------------------------------------------------------------------
    // 收藏初始化--------------------------------------------------------start
    var shoucang = $('.icon-fav, .icon2, .collect');
    ...
    shoucang.on('click', function () {
        if (!userid) {
            ...
            if ($(this).hasClass('on')) {
                ...
                $.get('/xf.d?m=addFav&userphone=' + userphone + '&username=' + username + '&userid=' + userid + '&city=' + vars.paramcity + '&newcode=' + vars.paramid + '&projname=' + fangName + '&image=' + vars.outdoorpic + '&price=' + vars.xfinfoprice + '&address=' + vars.xfinfoaddress + '&math=' + Math.random(),
                    function (data) {
                        if (data.root.code === '100') {
    ...
    function click() {
        ...
        // 在线咨询
        $('.zaixianzixun, .chatxfnew, .icon1').on('click', function () {
        ...
        $('.fixTelBox').on('click', function (e) {
            if(e.target.className == 'fixTelBox' || e.target.className == 'close') {
                $('.fixTelBox').hide();
            }
        });
    ...
    // 主力户型控制星星亮
    $('#huxingslider .star-s').each(function () {
    ...
    $('.dongtai, .luntan, .zsList').css({display: 'block', width: '335px'});
    var Swiper6 = require('swipe/3.10/swiper');
    var dpdtlt = Swiper6('#ddlslider', {
        loop: true,
        onSlideChangeEnd: function (swiper) {
            var navtab = $('.DP, .DT, .LT, .ZS'),
    ...
    $('.DP, .DT, .LT, .ZS').click(function () {
        ...
        if ($(this).hasClass('ZS') && $('.ZS').hasClass('active')) {
            var hrefload = $('.ZS').attr('data-href');
            window.location = hrefload;
        }
	// （seo专用）
        if ($(this).hasClass('LT') && $('.LT').hasClass('active')) {
    ...
    function ckeckLogin() {
        if (userid && userphone) {
            ...
            alert('请登录后点评！');
            window.location.href = 'https://m.fang.com/passport/login.aspx?burl=/xf_2014/houseComment.jsp%3fcity=' + vars.paramcity + '%26id=' + vars.paramid;
            return;
        } else if (!userphone) {
            alert('只有手机验证用户才可以发表楼盘点评哦，先去验证吧');
            window.location.href = '/user.d?m=phonepage&city=' + vars.paramcity + '&mstype=bindphone&burl=/xf_2014/houseComment.jsp%3fcity=' + vars.paramcity + '%26id=' + vars.paramid;
            return;
        }
    }
    ...
    function zygwajax() {
        ...
        if (telbua.indexOf('android') !== -1 && (telbua.indexOf('zte') !== -1 || telbua.indexOf('samsung') !== -1 || telbua.indexOf('lenovo') !== -1 || telbua.indexOf('sm-') !== -1 || telbua.indexOf('gt-') !== -1 || telbua.indexOf('nexus') !== -1 || telbua.indexOf('sch-') !== -1)) {
            $('.tel').click(function () {
            ...
            function (data) {
                if (data) {
                    if (data.root.code === '100') {
                        ...
                        $redBagReward.find('li').eq($rbid.attr('index')).children('a').attr('href', '//m.fang.com/house/ec/RedBagDetail/Index?redBagId=' + $rbid.val() + '&phone=' + userphone).unbind('click') + '&EB_BehaviorID=SFHB0002';
                        ...
                        function (data) {
                            if (data) {
                                ...
                                if (data.root.code === '100') {
                                    ...
                                    $('#applyhb').hide();
                                    $redBagReward.find('li').eq($rbid.attr('index')).attr('class', 'used');
                                    $redBagReward.find('li').eq($rbid.attr('index')).find('a').attr('href', '//m.fang.com/house/ec/RedBagDetail/Index?redBagId=' + $rbid.val() + '&phone=' + phone + '&EB_BehaviorID=SFHB0002').unbind('click');
                                    getPartActive(city, newcode);
                                } else if (data.root.code === '010') {
                                    showMessage('亲已经领取过该红包啦~');
                                    hidePop('#applyhb');
                                    $redBagReward.find('li').eq($rbid.attr('index')).attr('class', 'used');
                                    $redBagReward.find('li').eq($rbid.attr('index')).children('a').attr('href', '//m.fang.com/house/ec/RedBagDetail/Index?redBagId=' + $rbid.val() + '&phone=' + phone + '&EB_BehaviorID=SFHB0002').unbind('click');
                                    getPartActive(city, newcode);
                                } else {
                                    bindClickRedbag();
                                    showMessage(data.root.message);
            ...
            if (data) {
                ...
                for (var i = 0; i < messarr.length; i++) {
                    ...
                    if (m && m !== '0') {
                        ...
                        $lia.attr('href', '//m.fang.com/house/ec/RedBagDetail/Index?redBagId=' + $lia.attr('rbid') + '&phone=' + data.root.code + '&EB_BehaviorID=SFHB0002').unbind('click');
                    } else {
                        $lia = $redBagReward.find('li').eq(i).find('a');
                        $lia.unbind('click');
        ...
        $.get('/xf.d?m=allActive&id=' + id + '&city=' + city + '&type=xfinfo', function (data) {
            if (data.trim()) {
                ...
                if (!userphone) {
                    ...
                    $redBagReward.find('li').each(function () {
                        ...
                        if ($this.attr('class') !== 'used') {
                            ...
                            $this.find('a').attr('href', '//m.fang.com/house/ec/RedBagDetail/Index?redBagId=' + $this.find('a').attr('rbid') + '&phone=' + userphone + '&EB_BehaviorID=SFHB0002');
                        }
                    });
                }
                ...
                $('#rebbaginfo').click(function () {
                    ...
                    window.location.href = '//m.fang.com/house/ec/RedBagDetail/Index?redBagId=' + $rbid.val() + '&phone=' + userphone + '&EB_BehaviorID=SFHB0002';
                });
            } else {
                // 如果没有红包活动，则显示购房团购
                $('#gftg').show();
    ...
    // 直播预告
    $.get('/xf.d?m=liveNotice&newcode=' + vars.paramid, function (data) {
        if (data) {
    ...
    module.exports = {
        init: function () {
            loadmiaosha(vars.paramid, vars.last, vars.paramcity);
            scajax();
            click();
            zygwajax();
