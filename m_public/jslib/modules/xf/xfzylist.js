define('modules/xf/xfzylist',
    ['jquery', 'util/util','hslider/1.0.0/hslider', 'lazyload/1.9.1/lazyload', 'footprint/1.0.0/footprint',
        ...
        'modules/xf/ZhiYeGuWen','modules/xf/XfTongJi', 'yanzhengma/1.0.0/yanzhengma'],
    function (require) {
        'use strict';
        var $ = require('jquery');
        ...
        var typehf;
        if (vars.zytab.length > 0) {
            typehf = '&type=xfzy&hf=' + vars.zytab;
        } else {
            typehf = '&type=xfzy';
        }
        ...
        function goPrice(type) {
			...
			if (type == 'dj') {
				if (Number(pricemin) === 0 && !pricemax) {
                    var url1 = vars.character + vars.purposexf + vars.saleDate + vars.orderby + vars.round + vars.fitment
                        + vars.railway + vars.comarea + vars.station + vars.bedrooms + vars.purposeArea;
                    url1 = url1? url1 + '/' : url1;
					window.location.href = '/xf/' + vars.paramcity + vars.district + '/' + url1 + tags + '?red=hb' + query + typehf;
				} else {
					window.location.href = '/xf/' + vars.paramcity + vars.district + '/pr' + pricemin
						...
						+ vars.fitment + vars.railway + vars.comarea + vars.station + vars.bedrooms + vars.purposeArea + '/' + tags + '?red=hb' + query + typehf;
				}
			} else {
				if (Number(pricemin) === 0 && !pricemax) {
					var url1 = vars.character + vars.purposexf + vars.saleDate + vars.orderby + vars.round + vars.fitment
					...
					window.location.href = '/xf/' + vars.paramcity + vars.district + '/' + vars.character + vars.purposexf + vars.saleDate + vars.orderby + vars.round
						...
						+ ',' + pricemax + '/' + tags + '?red=hb' + query + typehf;
				}
			}
        }
        ...
        function checkLocation(sort) {
			...
			//                                                    价格  热门     类别            开盘         排序    环线  学校类型 学区  装修 地铁       地铁站      商圈    户型    面积 总价
            var b = '/xf/' + vars.paramcity + district + '/' + price + remen + a + saleDate + orderby + round + xq + fitment + railway + station + comarea + room + sell + zongjia + mianji + flag + '?red=hb' + typehf;
        ...
        require.async('//clickm.fang.com/click/new/clickm.js', function () {
            Clickstat.eventAdd(window, 'load', function (e) {
                Clickstat.batchEvent('wapxfzysy_','');
            });
        });
		...
		$('#characterChioce dd').on('click', function () {
			...
			if ($this.attr('value') && $('#characterChioce .active').length !== 1) {
		...
		// 显示底部筛选
		//$('.sf-maskFixed .con').addClass('sfmaskFixedCon');
