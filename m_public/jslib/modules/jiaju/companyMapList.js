 ...
 * @modify 袁辉辉 增加"去这里"以及标点样式更新
 */
define('modules/jiaju/companyMapList', ['jquery', 'mapAPI/1.0.0/mapAPI'], function (require, exports, module) {
    'use strict';
    ...
    var SFMap = {
        api: {
            // 不移动地图时获取公司坐标ajax
            GET_COMPANYMAPLIST: 'ajaxCompanyMapList',
            // 移动地图时的ajax
            GET_COMPANYMAPCHANGE: 'ajaxMoveMap'
        },
        ...
        init: function () {
            ...
            var markerFunc = function (map) {
                ...
                allmap.off('touchend').on('touchend', '.m-zx', function (e, isCodeTrigger) {
                    ...
                    if (!isCodeTrigger) {
                        // 设置居中
                        map.setCenter(new BMap.Point($this.data('x'), $this.data('y')));
                    }
                    // 恢复为非active样式
                ...
                allmap.on('touchend', '.map-compName3', function (e) {
                    e.preventDefault();
                    var $this = $(this), detailUrl, from = vars.from ? '&from=' + vars.from : '', src = vars.src ? '&src=' + vars.src : '';
                    // 点击不为去这里
                    if (e.target.id !== 'go') {
                        var rewriteFrom = vars.from ? vars.from : '', rewriteSrc = vars.src ? ('_' + vars.src) : '_1';
                        detailUrl = vars.rewriteFlag ? vars.protocol + vars.jiajuSite
                        + 'company/' + $this.data('compid') + '/' + vars.city + '/' + rewriteFrom + rewriteSrc + '/'
                            : vars.protocol + vars.jiajuSite + '?c=jiaju&a=newCompanyDetail&companyid=' + $this.data('compid')
                        + '&city=' + vars.city + from + src;
                        ...
                        if (vars.userLat && vars.userLng) {
                            ...
                            detailUrl = 'http://api.map.baidu.com/direction'
                                ...
                                + '&destination=' + marker.data('y') + ',' + marker.data('x')
            ...
            var buildMarkerDomFunc = function (info) {
                ...
                // 自定义覆盖物
                var html = ''
                    + '<div class="m-zx" data-x="' + info.Lng + '" data-y="' + info.Lat + '"  style="left: -11px;top: -24px;"></div>'
                    + '<div class="map-compName3 none" style="left: -145px;bottom: 30px;" data-compid="' + (info.CompanyId || '') + '">'
                    + '<a href="javascript:void(0)">'
                    + '<div class="map-compTxt">'
                    + '<div class="wzxx">'
                    + '<h3 class="name"><span class="flor range">' + info.Distance
                    + '</span>' + info.CompanyName + '</h3>'
                    + '<p>' + info.Address + '</p>'
                    + '</div>'
