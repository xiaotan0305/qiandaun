 ...
 */
define('modules/bask/answerRightNow', ['jquery'], function (require, exports, module) {
    'use strict';
    module.exports = function () {
        ...
        $('.submit').on('click', function () {
            ...
            if (content === '' || content === '在这输入回答，被采纳最多获40积分') {
                ...
                if (answerId === '') {
                    // 没有登录
                    burl = vars.askSite + '?c=bask&a=answerRightNow&id=' + askId + '&zhcity=' + vars.zhcity + '&grouptype=' + vars.grouptype + '&src=client';
                    window.location.href = vars.loginUrl + '?burl=' + encodeURIComponent(burl);
                } else {
                    // 对提交内容的判断
                    // ajax提交内容，并且做提交的判断
                    $.get(vars.askSite + '?c=bask&a=ajaxPostAnswer&id=' + askId + '&answer_id=' + answerId + '&source=' + source + '&answer_ip='
                        + answerIp + '&content=' + content + '&r=' + Math.random(), function (returnData) {
                        var data = returnData;
                        if (data.code === '100') {
                            // 跳转到该问题详情页，并且定位
                            window.location.href = vars.askSite + '?c=bask&a=detail&id=' + askId + '&zhcity=' + vars.zhcity + '&grouptype=' + vars.grouptype + '&src=client';
                            // 回答成功，用户行为。
