...
(function (w, f) {
    ...
    function SelectorClass(ops) {
        ...
        this.area = ops.area;
        this.defaultSelect = ops.defaultSelect;
        this.singleLiHeight = ops.singleLiHeight;
        this.areaCancelFunc = ops.areaCancelFunc;
        this.getSelection = ops.getSelection;
        this.inited = false;
        // ul列表绑定事件
    ...
    SelectorClass.prototype = {
        ...
        setOneShow: function () {
            ...
            for (var i = 0; i < arrL; i++) {
                if (that.area[i] === that.defaultSelect) {
                    // 判断是默认选项
                    areaLi[i] = '<li class = "activeS" >' + that.defaultSelect + '</li>';
                    // 记录默认选项在列表中的索引
                    that.defaultIndex = i;
                } else {
                    areaLi[i] = '<li>' + that.area[i] + '</li>';
                }
            }
            ...
            for (var key in area) {
                if (area.hasOwnProperty(key)) {
                    if (key === that.defaultSelect) {
                        // 判断是默认选项
                        areaLi[i] = '<li id = ' + i + ' class = "activeS" >' + that.defaultSelect + '</li>';
                        that.defaultIndex = i;
                        // 记录默认选项在列表中的索引
                    } else {
                        areaLi[i] = '<li id = ' + i + '>' + key + '</li>';
                    }
                    i++;
                }
            }
        ...
        ulbindClick: function () {
            ...
            that.cityList.on('click', 'li', function () {
                ...
                if (that.level === 'one') {
                    ...
                    // 给被点击的列表添加动态样式效果
                    clickLi.on('webkitAnimationEnd', function () {
                        // 监听动画结束事件
                        that.oneClickLiListener(clickLi);
                        clickLi.off('webkitAnimationEnd');
                    });
                    clickLi.on('mozAnimationEnd', function () {
                        // 监听动画结束事件
                        that.oneClickLiListener(clickLi);
                        clickLi.off('mozAnimationEnd');
                    });
                    clickLi.on('animationend', function () {
                        // 监听动画结束事件
                        that.oneClickLiListener(clickLi);
                        clickLi.off('animationend');
                    });
                    // 点击函数回调 返回被选择的内容
                    that.getSelection && that.getSelection(clickLi.text());
                } else if (that.level === 'two') {
                    // 一级列表的点击监听事件
                    // 返回按钮
                    that.twoTxt = clickLi.text() + ' ';
                    that.returnBtn.show();
                    // 显示返回按钮
                    clickLi.addClass('active').siblings().removeClass('activeS');
                    // 添加点击时候的动态效果
        ...
        twoFirstClickLiListener: function (clickLi) {
            ...
            if (cityCon.length) {
                ...
                that.createSecondList(clickLiId, clickLi);
                // 如果二级列表不存在，则创建二级列表
            }
        },
        /**
         ...
         * @param clickLi 被点击的一级列表
         */
        createSecondList: function (clickLiId, clickLi) {
            var that = this;
            var cityArr = that.area[clickLi.text()];
            // 一级列表对应的二级列表的数组
            var cityDom = '';
            // 生成的二级列表的dom
            cityDom = '<div id="cityCon_' + clickLiId + '" class="con conSecond show">' +
                '<ul>';
            for (var j = 0; j < cityArr.length; j++) {
                cityDom += '<li>' + cityArr[j] + '</li>';
            }
            cityDom += '</ul></div>';
            ...
            $('#cityCon_' + clickLiId).on('click', 'li', function (ev) {
                ...
                // 添加选择内容
                that.twoTxt += secondClickLi.text();

                secondClickLi.addClass('active');
                // 添加点击时候的动态效果
                secondClickLi.on('webkitAnimationEnd', function () {
                    that.twoSecondClickLiListener(secondClickLi, clickLi, clickLiId);
                    secondClickLi.off('webkitAnimationEnd');
                });
                secondClickLi.on('mozAnimationEnd', function () {
                    that.twoSecondClickLiListener(secondClickLi, clickLi, clickLiId);
                    secondClickLi.off('mozAnimationEnd');
                });
                secondClickLi.on('animationend', function () {
                    that.twoSecondClickLiListener(secondClickLi, clickLi, clickLiId);
                    secondClickLi.off('animationend');
                });
                that.getSelection && that.getSelection(that.twoTxt);
                // 返回被选择的内容
            });
        },
        /**
        ...
        showSelector: function (ops) {
            ...
            that.area = ops.area;
            that.defaultSelect = ops.defaultSelect;
            that.singleLiHeight = ops.singleLiHeight;
            that.areaCancelFunc = ops.areaCancelFunc;
            that.getSelection = ops.getSelection;
            that.defaultIndex = '';
            that.init();
        }
    };
