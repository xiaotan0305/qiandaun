define('jwingupload/imageCompresser', ['jwingupload/jpegMeta', 'jwingupload/JPEGEncoder'], function(require, exports, module) {
    "require:nomunge,exports:nomunge,module:nomunge";
    var jpegMeta = require('jwingupload/jpegMeta').JpegMeta;
    var JPEGEncoder = require('jwingupload/JPEGEncoder');
    module.exports = {init: function() {
        var ImageCompresser = {isIosSubSample: function(img) {
            ...
            var canvas = document.createElement('canvas'), ctx = canvas.getContext('2d'), data, sy = 0, ey = h, py = h;
            canvas.width = 1;
            canvas.height = h
            ...
            opts = jQuery.extend({maxW: 800,maxH: 800,quality: 0.8,orien: 1}, opts);
            ...
            var canvas = document.createElement('canvas'), ctx = canvas.getContext('2d'), base64Str;
            this.doAutoRotate(canvas, w, h, opts.orien);
            if (jq.os.ios) {
                var tmpCanvas = document.createElement('canvas'), tmpCtx = tmpCanvas.getContext('2d'), d = 1024, vertSquashRatio = ImageCompresser.getIosImageRatio(img, _w, _h), sx, sy, sw, sh, dx, dy, dw, dh;
            ...
            if (jq.os.android) {
                var imgData = ctx.getImageData(0, 0, canvas.width, canvas.height), encoder = new JPEGEncoder.JPEGEncoder(quality * 100);
