...
(function (window, factory) {
    ...
    if (typeof define === 'function') {
        // AMD
        define('jwingupload/1.0.4/jwingupload', ['jquery'], function (require) {
            return factory(window, require('jquery'));
        });
    } else {
        // browser global
        window.jWingUpload = factory(window, window.jQuery);
    }
})(window,function(window, $){
    var win = window,document = win.document,hostname = win.location.hostname.split("."),len = hostname.length, version = "1.0.0", core_Ids = [], jWingUpload, isFileInputSupported;
