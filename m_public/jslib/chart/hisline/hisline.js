...
(function (w, f) {
    ...
    if (typeof define === 'function') {
        define('chart/hisline/hisline', ['chart/raf/1.0.0/raf'], function (require) {
    ...
    HisLine.prototype = {
        init: function () {
            ...
            // 设置刻度左侧canvas及绘图api
            that.scaleCvsL = $('<canvas height="' + that.options.h * 2 + '" style="height:' + that.options.h + 'px"></canvas>');
            ...
            // 设置刻度右侧canvas及绘图api
            that.scaleCvsR = $('<canvas height="' + that.options.h * 2 + '" style="height:' + that.options.h + 'px"></canvas>');
        ...
        fillLineData: function (arr) {
            ...
            var cssObj = {
                ...
                // borderRight: that.options.bgLinePx + 'px solid ' + that.options.bgLineColor,
                'z-index': 1
            };
            if (that.maxValR === 0) {
                cssObj.display = 'none';
            ...
            for (i = 0; i <= that.options.scaleNumber; i++) {
                ...
                var scaleVal = that.options.lineBit ? (maxVal - dValue).toFixed(2) : String(Math.floor(maxVal - dValue));
                ...
                that.scaleRDrawApi.fillText(scaleVal + that.options.lineBit, 0, my);
                that.scaleRDrawApi.restore();
            }
        ...
        setCvsWidth: function () {
            ...
            ccObj['margin-left'] = that.scaleLTextW / 2 + 'px';
            that.cvs.css(ccObj);
        ...
        fillHisData: function (arr) {
            ...
            for (i = 0; i < that.dataHisL; i++) {
                ...
                var x = partW * (i + 0.5);
                // 与最大值作对比算出每个柱子的高度
                var h = (valArr[i] - that.minValL) / (that.maxValL - that.minValL) * that.chartH + that.options.downSp;
        ...
        drawVerticalLine: function (x) {
            var that = this,
                mx = 99999,
                index = 0;
            for (var i = 0, len = that.bgLineArr.length; i < len; i++) {
                var tmpMin = Math.abs(x - that.bgLineArr[i].vertical);
                if (mx > tmpMin) {
                    mx = tmpMin;
                    index = i;
                }
            ...
            that.drawBg(that.options.lineArr);
            that.drawHis();
            that.drawDottedLine(that.bgLineArr[index].vertical, 0, that.bgLineArr[index].vertical, that.chartTotalH, 10);
            that.drawLine();
        },
                ...
                if (mx > (data.x - data.w / 2) / that.scale && mx < (data.x + data.w / 2) / that.scale) {
                    ...
                    if (data.href) {
                        ...
                        var hisVal = parseInt(data.value),
                            lineVal = parseInt(that.dataLinePos[i].value);
                        // 兼容没有值的情况
                        if (hisVal || lineVal) {
