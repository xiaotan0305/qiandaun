...
(function (w, f) {
    ...
    if (typeof define === 'function') {
        define('chart/histogram/1.0.0/histogram', [], function () {
            return f(w);
        });
    ...
    histogram.prototype = {
        _init: function () {
            ...
            if (this.options.side) {
                ...
                data = '<ul class="histogramData" style="position:absolute;z-index:0;left:0;top:0;width:100%;word-break: break-all;text-align: center;font-size: 10px;">';
                ...
                for (i = 0; i < l; i++) {
                    ...
                    data += '<li style="width:' + wPer + '%;position: absolute;left:' + wPer * i
                        + '%;top:0;"><p style="height:' + smh + 'px;position:relative;width:100%;"><span style="position:absolute;left:0;bottom:0;width:100%;">'
                        + this.options.data[i].value + '</span></p><p style="margin-top:' + sh + 'px;border-top:1px solid #ccc">'
                ...
                data = '<ul class="histogramData" style="word-break: break-all;text-align: center;font-size: 10px;position:absolute;z-index:0;left:0;top:0;width:100%;">';
                l = this.options.data.length;
                wPer = Math.floor(100 / l);
                for (i = 0; i < l; i++) {
                    sh = Math.floor(rate * (Math.abs(parseFloat(this.options.data[i].value)) * 100) / 100);
                    if (parseFloat(this.options.data[i].value) >= 0) {
                        ...
                        data += '<li style="width:' + wPer + '%;position: absolute;left:' + wPer * i + '%;top:0;"><p style="height:'
                            + smh + 'px;position:relative;width:100%;"><span style="position:absolute;left:0;bottom:0;width:100%;">'
                            + this.options.data[i].value + '</span></p><p style="margin-top:' + sh + 'px;border-top:1px solid #ccc;">'
                            + this.options.data[i].name + '</p></li>';
                    } else {
                        ...
                        data += '<li style="width:' + wPer + '%;position: absolute;left:' + wPer * i + '%;top:0;"><p style="height:' + (smh + 1)
                            + 'px;position:relative;width:100%;border-bottom:1px solid #ccc;"><span style="position:absolute;left:0;bottom:0;width:100%;">'
                            + this.options.data[i].name + '</span></p><p style="margin-top:' + sh + 'px">' + this.options.data[i].value + '</p></li>';
                    }
