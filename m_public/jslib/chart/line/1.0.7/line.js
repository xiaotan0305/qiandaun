/**
 * 在1.0.2的基础上，增加了弹出浮层功能，在有alertDom这个参数的情况下，会弹出浮层。
 * alertDom为弹出的浮层的dom元素
 * 此形式为多条曲线，并且可能存在预测
 * by blue
 * @param {Object} w
 * @param {Object} f
...
(function (w, f) {
    ...
    if (typeof define === 'function') {
        define('chart/line/1.0.7/line', ['chart/raf/1.0.0/raf', 'iscroll/2.0.0/iscroll-lite'], function (require) {
    ...
    Line.prototype = {
        init: function () {
            ...
            // 如果有传浮层dom元素，则对canvas进行事件绑定
            if (that.options.alertDom) {
                that.chartCvs.on('touchend', function (e) {
                    if (!that.options.alertDom.is(':hidden')) {
                        that.options.alertDom.hide();
                    }
                    // that.point为鼠标点击时的坐标
                    that.point = that.getEventPosition(e);
                    that.chartDrawApi.clearRect(0, 0, that.chartDrawApi.canvas.width, that.chartDrawApi.canvas.height);
                    that.drawLine(that.point);
                    that.fillData(that.point);
                });
            }
        },
        ...
        getEventPosition: function(ev){
            ...
            if (ev.pageX || ev.pageX == 0) {
                ...
                // clientY会莫名其妙的多出420
                x = ev.clientX + Math.abs(that.chartCvs.css('transform').split(',')[4].trim());
                y = ev.clientY;
            }
            return {x: x, y: y};
        },
         ...
         * @param data 第几条线
         */
        drawAlert: function (point, data, times) {
            var that = this;
            // 设置浮层
        ...
        drawBg: function (page) {
            ...
            for (var i = 0; i < that.xAxisL; i++) {
                ...
                if (page && Math.abs(page.x - mx/2) < 6) {
                    that.chartDrawApi.strokeStyle = 'red';
                }
        ...
        drawLine: function (point) {
            ...
            for (var i = 0; i < that.dataArrL; i++) {
                ...
                // 根据走势线的值数据画一条曲线
                that.drawOneline(that.dataArr[i].yAxis, color, that.dataArr[i].forecast, point, times);
                colorIdx++;
                times++;
            }
        },
         ...
         */
        drawOneline: function (arr, color, forecast, point, times) {
            var that = this;
            // 该走势线的点个数
            var l = arr.length;
            // 循环使用
            ...
            for (; i < l; i++) {
				...
				if (!arr[i]) {
					my = '';
					mz = (1 - (Number(arr[i+1]) - that.minVal) / (that.maxVal - that.minVal)) * that.chartH + that.options.chartSp;
				}
                p.push({x: mx, y: my, z: mz});
            }
            ...
            for (i = 0; i < l; i++) {
                ...
                if (point) {
                    if (Math.abs(point.x - that.conData[i][0].x / 2) < 6) {
                        that.drawAlert(point, that.conData[i][0].data, times);
                    }
                }
                if (i !== l - 2 || !forecast) {
                    that.chartDrawApi.save();
