 ...
 */
define('imageUpload/1.0.0/imageUpload', ['jquery', 'imageUpload/jpegMeta','imageUpload/imageCompresser'], function (require) {
    'use strict';
    var $ = require('jquery'),
    ...
    ImageUpload.prototype = {
        ...
        addBlock: function (fileObj, loading) {
            ...
            if (loading) {
                ...
                if (that.options.loadingClass) {
                    ...
                    fileObj.dom.img.show().attr('src', loadingGif);
                }
            } else {
        ...
        onSuccess: function (fileObj, data) {
            ...
            if (data.result) {
                ...
                that.addBlock(fileObj);
                // bbs图片上传，前台预览需要收敛后的图片地址imgurl，但是评论的内容包括图片地址需要给接口传没收敛的图片地址simgurl
                if (data.res) {
                    fileObj.simgurl = data.res;
                }
                that.imgsArray.push(fileObj);
                seajs.emit('numChange', that.imgsArray.length);
            } else {
