...
(function (factory) {
    ...
    if (typeof define === 'function') {
        define('imageUpload/imageCompresser', ['jquery'], function (require) {
            var $ = require('jquery');
            return factory($);
        });
    } else {
        window.ImageCompresser = factory(jQuery);
    }
})(function($){
    ...
    var ImageCompresser = {
        ...
        getImageBase64: function (img, opts) {
            opts = $.extend({maxW: 800, maxH: 800, quality: 0.8, orien: 1}, opts);
            ...
            if (os.android) {
                var imgData = ctx.getImageData(0, 0, canvas.width, canvas.height), encoder = new JPEGEncoder(quality * 100);
