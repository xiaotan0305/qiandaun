 ...
 * by blue
 */
define('search/esfhd/xiaoquSearch', ['jquery', 'search/search'], function (require, exports, module) {
    'use strict';
    // jquery索引
    var $ = require('jquery');
    // 公共方法对象索引
    ...
    function XiaoquSearch() {
        // 弹窗html字符串
        this.html = '<div id="popBtn" class="searchPage" style="position:relative;z-index:9999">'
            + '<header class="header">'
        ...
        // 显示弹窗的按钮
        this.showPopBtn = '#CFJ_searchtext';
        // 返回按钮
    ...
    XiaoquSearch.prototype.createAutoPromptList = function (inputValue) {
        var url = vars.esfSite + '?c=esfhd&a=ajaxGetLoupanList';
        var obj = {
            city: vars.city,
            q: inputValue,
        };
    ...
    XiaoquSearch.prototype.search = function (obj) {
        ...
        that.back();
        if(vars.localStorage.getItem('top')){
            var top = parseInt(vars.localStorage.getItem('top'));
            $('body').scrollTop(top);
        }
    };
