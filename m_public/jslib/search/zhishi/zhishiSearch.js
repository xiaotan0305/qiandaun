...
define('search/zhishi/zhishiSearch', ['jquery', 'search/search'], function (require, exports, module) {
     ...
     * @constructor
     */
    function ZhishiSearch() {
        // 设置点击id的前缀
        var prefix = 'wap';
        // 部署click的id
        if (vars.action === 'getClassList' && vars.jtname === 'xf') {
            prefix += 'gfznlist';
        } else if (vars.action === 'index') {
            if (vars.jtname === 'jiaju') {
                prefix += 'gjiajuzssy';
            } else if (vars.jtname === 'xf') {
                prefix += 'zhishisy';
            }
        } else if (vars.action === 'detail') {
            if (vars.jtname === 'xf') {
                prefix += 'gfznxq';
            } else if (vars.jtname === 'jiaju') {
                prefix += 'gjiajuzsxq';
            }
        }
        ...
        html = '<div class="searchPage">'
            + '<header class="header">'
            + '<div class="left" id="' + prefix + '_D01_08"><a href="javascript:void(0);" class="back"><i></i></a></div>'
            + '<div class="cent"><span>知识搜索</span></div>'
            + '</header>'
            + '<form class="search" action="" onsubmit="return false;" method="get" autocomplete="off">'
            + '<div class="searbox">'
            + '<div class="ipt" id="' + prefix + '_D01_09">'
            ...
            + '</div>'
            + '<a href="javascript:void(0);" id="' + prefix + '_D01_18" class="btn" rel="nofollow"><i></i></a>'
            + '</div>'
            + '</form>'
            + '<div class="searLast" id="' + prefix + '_D01_12">'
            ...
            + '<div class="searList" id="' + prefix + '_D01_10"><ul></ul></div>'
            + '<div class="clearBtn" id="' + prefix + '_D01_11">'
        ...
        this.historyMark = vars.city + 'zhishiHistory';
        // 显示弹窗的按钮
        this.showPopBtn = '#new_searchtext,#searchid .btn';
        // 返回按钮
     ...
     */
    $.extend(ZhishiSearch.prototype, search);

    /**
     * 重写创建自动提示列表方法，传入需要的接口地址及接口传参
     * @param inputValue
     */
    ZhishiSearch.prototype.createAutoPromptList = function (inputValue) {
        var url = vars.zhishiSite + '?c=zhishi&a=ajaxGetSearchTip';
        var obj = {
            kw: inputValue,
            city: vars.city
        };
     ...
     */
    ZhishiSearch.prototype.getAutoPromptListContent = function (dataArr) {
        var that = this;
        ...
        for (var i = 0; i < dataArrL; i++) {
            ...
            // 知识的搜索提示跳转到知识的详情页或者专题页面
            obj.jumpUrl = dataArr[i].searchurl;
            // 储存条件对象数据
     ...
     * 创建热词列表
     */
    ZhishiSearch.prototype.createHotSearch = function () {
        var that = this;
         ...
         * 调用获取热词数据接口
         */
        $.get(vars.zhishiSite + '?c=zhishi&a=ajaxGetPurchaseType', {
            city: vars.city, jtname: vars.jtname
        }, function (data) {
     ...
     * 增加热词点击和显示热词操作
     */
    ZhishiSearch.prototype.showPop = function () {
        var that = this;
     ...
     */
    ZhishiSearch.prototype.inputChange = function () {
        var that = this;
     ...
     * 增加显示热词和显示历史记录操作
     */
    ZhishiSearch.prototype.closeAutoPromptList = function () {
        var that = this;
     ...
     * 增加清除所有列表之后也清除热词列表
     */
    ZhishiSearch.prototype.clearAllList = function () {
        var that = this;
     ...
     * 清除热词
     */
    ZhishiSearch.prototype.clearHotSearch = function () {
        var that = this;
     ...
     * 增加之后重建热词列表
     */
    ZhishiSearch.prototype.deleteHistoryList = function () {
        var that = this;
     ...
     * 点击搜索按钮处理
     */
    ZhishiSearch.prototype.search = function () {
        var that = this;
        // 获取用户输入内容
     ...
     * @returns {string}
     */
    ZhishiSearch.prototype.getHistoryContent = function (arr) {
        var that = this,
            len = arr.length,
            html = '';
        // 普通显示条数的html字符串
     ...
     * @param obj
     */
    ZhishiSearch.prototype.clickListSearch = function (obj) {
        if (!obj)return;
        var that = this;
        // 将搜索关键字写入搜索input中
