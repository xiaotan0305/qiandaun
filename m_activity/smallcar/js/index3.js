...
$(function () {
    ...
    // 音效
    var bgm = new Audio();
    bgm.src = 'http://js.soufunimg.com/common_m/m_activity/smallcar/music/bg.mp3';
    if (typeof bgm.canPlayType !== 'function') {
        alert('您的浏览器不支持该游戏音频播放，请换个浏览器试试哦~');
    }
    var lbm = new Audio();
    lbm.src = 'http://js.soufunimg.com/common_m/m_activity/smallcar/music/lb.mp3';
    var lzm = new Audio();
    lzm.src = 'http://js.soufunimg.com/common_m/m_activity/smallcar/music/lz.mp3';
    var zdm = new Audio();
    zdm.src = 'http://js.soufunimg.com/common_m/m_activity/smallcar/music/zd.mp3';
    ...
    var baseUrl = 'http://js.soufunimg.com/common_m/m_activity/smallcar/images/';
    var canvasImages = [
        baseUrl + 'bg.png',
        // 小汽车
        baseUrl + 'car.png',
     ...
     * @param index 图片数据下标
     * @param fnSucc 成功回调函数
     * @param fnLoading 加载中回调函数
     */
    function overtime(imgArr, index, fnSucc, fnLoading) {
        index++;
        loadedNum.html(parseInt(index / imgArr.length * 100));
        fnLoading && fnLoading();
        if (index === imgArr.length) {
            ...
            loadPic(imgArr, index, fnSucc, fnLoading);
        }
    }
     ...
     * @param fnLoading 加载中回调函数
     */
    function loadPic(imgArr, index, fnSucc, fnLoading) {
        fnSucc = fnSucc || null;
        fnLoading = fnLoading || null;
        var len = imgArr.length;
        var oImg = new Image();
        oImg.src = imgArr[index];
        clearTimeout(loadTimer);
        loadTimer = setTimeout(function () {
            overtime(imgArr, index, fnSucc, fnLoading);
        }, 4000);
        oImg.onload = function () {
            clearTimeout(loadTimer);
            var imgName = imgArr[index].split('/images/') ? imgArr[index].split('/images/')[1] : '';
            if (imgName) {
    ...
    loadPic(
        ...
        function () {
            ...
            // 背景图片运动
            var bgCon = new BgCon(imgJson.bg, cWidth, cHeight, true);
            //stage.addCild(bgCon);
            ...
            stage.addCild(lbNum);
            // 礼包数图片
            var leftBg = new PicCon(imgJson.lbnum, 15 / 500 * cWidth, 20 / 900 * cHeight, 50 / 500 * cWidth, 25 / 900 * cHeight, true);
            ...
            // 左右按钮
            var leftBtn = new PicCon(imgJson.leftbtn, leftBtnMsg.x, leftBtnMsg.y, leftBtnMsg.w, leftBtnMsg.h, true);
            stage.addCild(leftBtn);
            var rightBtn = new PicCon(imgJson.rightbtn, rightBtnMsg.x, rightBtnMsg.y, rightBtnMsg.w, rightBtnMsg.h, true);
            stage.addCild(rightBtn);

            // 小汽车
            var car = new PicCon(imgJson.car, carMess.x, carMess.y, carMess.w, carMess.h, true);
            //stage.addCild(car);
            // 碰撞炸弹效果
            var gameover = new PicCon(imgJson.bzpic, eleMess.bzpic.x, eleMess.bzpic.y, eleMess.bzpic.w, eleMess.bzpic.h, true);
            // 碰撞礼包效果
            ...
            function createEle(name) {
                ...
                var rndSd = 's' + rnd(1, 4);
                if (name !== 'over') {
                    var item = new Element(imgJson[name], sd[rndSd], eleMess[name].y, eleMess[name].w, eleMess[name].h, name);
                    arr.push(item);
                }
            }
            ...
            function sendResult() {
                ...
                if (lbText.text > 0) {
                    // http://m.test.fang.com/jiaju/?c=jiajuact&a=ajaxCarGameJfAndCoupon&count=111
                    var url = '/jiaju/?c=jiajuact&a=ajaxCarGameJfAndCoupon';
                    $.ajax({
                        url: url,
                        dataType: 'json',
                        data: {
                        ...
                        success: function (data) {
                            ...
                            // 微信分享
                            window.weixin.updateOps({
                                shareTitle: '像素小赛车我跑了' + lenText.text + '米，不信你能超过我!'
                            });
                        },
