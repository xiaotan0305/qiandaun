...
$(function () {
    ...
    activityEnd.setFullYear(2016, 11, 2);
    ...
    if (askLen === 1) {
        initFirstP.html('接口返回数据失败，请刷新页面~').css({
            'font-size': '.5rem',
            'text-align': 'center',
            height: '1.2rem',
            'line-height': '.5rem'
    ...
    var oImgSrc = oImg.attr('data-src');
    // 图片随机版本号
    ...
    hbClose.on('click', function () {
        ...
        if (leftPrizeNumber <= 0) {
            ...
            hbmsg.html('<b>小编正加紧审核，积分将于3日内到账哦~</b>');
        }
        setTimeout(function () {
            hbClose.hide();
        }, 500);
