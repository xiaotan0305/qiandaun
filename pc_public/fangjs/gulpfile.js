...
let paths = {
    build: 'build',
    js: ['!./gulpfile.js', '!node_modules/**/*', '!fang/**/*', '!build/**/*'],
    fangjs: [`fang/${version}/fang.js`, `fang/${version}/combo.js`, `fang/${version}/config.js`, `fang/${version}/polyfill.js`]
};
// 自定义路径
let isFang = true;
if (yargs.p && typeof yargs.p !== 'boolean') {
    if (yargs.p !== 'fang') isFang = false;
    paths.js.unshift('./' + yargs.p + '/**/*.js');
} else {
    paths.js.unshift('./**/*.js');
}
...
gulp.task('rjs', function () {
    ...
    var arr = [gulp.src(src, {base: '.'},getFiles(src)), plumber(), concat(`fang${version}.js`), uglify({
        mangle: true,
        output: {
            ascii_only: true
        }
...
gulp.task('js', function () {
    ...
    var src = paths.js;
    var arr = [gulp.src(src,{base: '.'} ,getFiles(src, ['fang\/'])), plumber(), uglify({
        mangle: true,
        output: {
            ascii_only: true
        }
