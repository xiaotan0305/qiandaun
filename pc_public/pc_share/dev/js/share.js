...
(function (window, factory) {
    ...
    var base;
    var id;
    // base = "//static.soufunimg.com/common_m/pc_public/pcshare/";
    base = "//static.test.soufunimg.com/common_m/pc_public/pcshare/";
    // base = "//test.fang.com/wap_static/pc_public/pc_share/dev/";
    id = base + "js/share.js";
    if (typeof define === 'function') {
        // AMD
        define(id, [], function (require) {
            return factory(window, base, require)
        })
    } else {
        // browser global
        window.pcShare = factory(window, base)
    }
})(window, function factory(window, base, require) {
    'use strict';
    var encodeCom = encodeURIComponent;
    var imgPath = base + 'images/';
    /**
     * 截取字符串到固定长度
     * @param str 截取的字符串
     * @param len 截取长度
    ...
    function setAttr(className, attr, value) {
        var $obj = document.querySelector ? document.querySelector('.' + className) : document.getElementsByClassName(obj)[0];
    ...
    function pcShare(opt) {
        ...
        if (!(this instanceof pcShare)) {
            return new pcShare(opt)
        }
        var document = window.document;
        var title = document.title;
        var url = opt.url || window.location.href;
        // 如果当前传入的url 跟之前的url不同则更新信息
        if (url === oldUrl) {
            this.show();
            return;
        }
        ...
        if (this.options.channel) {
            this.options.source = '&platform=' + this.options.channel + (IsPC() ? 'pc' : 'wap');
        }
        this.creatHTML();
        this.show();
        var that = this;
        // 取消分享按钮
        var quitBtn = document.getElementsByClassName('pcshare_close')[0];
        // 取消分享
        ...
        var sendMailBtn = document.getElementsByClassName('pcshare_mail')[0];
        sendMailBtn.onclick = function () {
            that.sendMail();
        };
    }
    // 更新页面信息
    pcShare.prototype.update = function () {
        var that = this;
        // 更新微信扫码地址url
        var wxPic = 'http://u.soufun.cn/qrcode.php?url=' + encodeURIComponent(this.options.url) + '&type=newhouse&resize=82';
        setAttr('shareweixin', 'src', wxPic)
        // 更新clickId
        var clickArr = that.options.sign;
        var clickObj = {
            clickID0: '',
            clickID1: '',
            clickID2: '',
            clickID3: 'pcshare_copy',
            clickID4: 'pcshare_close'
        };
        if (Object.prototype.toString.call(clickArr) === "[object Array]" && clickArr.length) {
            for (var i = 0, len = clickArr.length; i < len; i++) {
                clickObj['clickID' + i] = clickArr[i];
            }
        }
        // 分享到微博id
        setAttr('pcshare_weibo', 'id', clickObj.clickID0)
        // 分享到qq空间
        setAttr('pcshare_qqzone', 'id', clickObj.clickID1)
        // 发邮件
        setAttr('pcshare_mail', 'id', clickObj.clickID2)
        // 复制
        setAttr('pcshare_copy', 'id', clickObj.clickID3);
        // 关闭按钮点击id
        setAttr('pcshare_close', 'id', clickObj.clickID3);
    }
    /**
     * 拼接显示分享的html结构
     *
    ...
    pcShare.prototype.tpl = function () {
        ...
        var tepl = ['<div class="layer_share" id="share_layer"></div>',
            ...
            '<span class="pcshare_close " id="' + clickObj.clickID4 + '"></span>',
            '<div class="share_icon">',
            '<div class="s_erwei share_icon_zi fl">',
            '<div class="share_tu">',
            '<img class="shareweixin" style="width:82px;height:82px" src="' + wxPic + '" alt="">',
            '</div><p class="f12 gray3">微信“扫一扫”<br>分享好友</p></div>',
            '<div id="' + clickObj.clickID0 + '" class="s_weibo share_icon_zi fl pcshare_iconbox pointer pcshare_weibo" name="wb">',
            '<div class="share_tu">',
            '<img src="' + imgPath + 'share_weibo.png" alt="">',
            '</div><p class="f12 gray3">点击分享到微博</p>',
            '</div>',
            '<div id="' + clickObj.clickID1 + '" class="s_qqzone share_icon_zi fl pcshare_iconbox pointer pcshare_qqzone" name="qz">',
            '<div class="share_tu">',
            '<img src="' + imgPath + 'share_qq.png" alt=""></div><p class="f12 gray3">点击分享到QQ空间</p></div>',
            '<div id="' + clickObj.clickID2 + '" class="s_mail share_icon_zi fl pointer pcshare_mail">',
            '<div class="share_tu">',
            '<img src="' + imgPath + 'share_mail.png" alt=""></div><p class="f12 gray3">点击发邮件</p></div>'
        ];
        if (that.options.copy) {
            tepl.push('<div id="' + clickObj.clickID3 + '" class="s_copy_link share_icon_zi fl pointer pcshare_copy"><div class="share_tu" style="border:none"><img src="' + imgPath + 'share_lianjia.png" alt=""></div><p class="f12 gray3" style="border:none">点击复制地址</p></div></div>');
        }
        tepl.push('<div class="pcshare_tishi_add" id="pcshare_tishi_add" style="display: none">地址已复制</div></div>');
        return tepl.join('')
    }
    ...
    pcShare.prototype.creatHTML = function () {
        var oDiv = document.getElementById('share_layer')
        ...
        s.innerHTML = '.share_tucon .f12{font-size: 12px;}' +
            '.share_tucon .pointer{cursor:pointer;}' +
    ...
    pcShare.prototype.copy = function () {
        var that = this;
        var copyBtn = document.getElementsByClassName('pcshare_copy')[0];
        var copyId = copyBtn.getAttribute('id');
    ...
    pcShare.prototype.sendMail = function () {
        ...
        window.open('http://pub.dmp.fang.com/mailshare?lpName=' + encodeCom(that.options.buildName) + '&lpLink=' + encodeCom(url));
    }
    return pcShare
})
