...
{
	var curcity = document.getElementById('curcity').innerHTML;
	get_location();
}
  			...
  			if (xmlHttp.status==200){
				...
				var encity = data.root.encity;
			   	...
			   	if(addr != null && addr != ''){
					...
					addCookie("zhcity",city,24*30);
			   		addCookie("encity",encity,24*30);
			   		addCookie("addr",addr,24*30);
		...
		if (xmlHttp.readyState==4){
  			if (xmlHttp.status==200){
				...
				var encity = data.root.encity;
			   	...
			   	if(addr != null && addr != ''){
					if(curcity != encity && getCookie("encity") != encity){
						var conf =  confirm("定位显示您当前的城市是"+city+"，是否需要切换城市？");
						addCookie("zhcity",city,24*30);
				   		addCookie("encity",encity,24*30);
				   		addCookie("addr",addr,24*30);
						if(conf != "0" && city != null && city != ''){
							window.location = "/"+encity+".html";
						}
					};
					addCookie("zhcity",city,24*30);
			   		addCookie("encity",encity,24*30);
			   		...
			   		if(conf != "0"){
						window.location = "/city/hotcity.jsp";
					}
			   	}
			}
		}
