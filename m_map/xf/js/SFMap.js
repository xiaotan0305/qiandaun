...
var SFMap = {
    ...
    searchHouse: function() {
        ...
        var params = {'c': 'map', 'a': 'ajaxXfMapSearch', 'city': encity, 'output': 'json', 'zoom': zoom};
    ...
    showResult: function(hits,yidicity) {
        var me = this;
        var metaMarkers = [];
        if (!hits)
            return;
        for (var i = 0; i < hits.length; i++) {
            ...
            if (!(typeof(yidicity) == "undefined")) {
                info.m_houseurl = 'http://m.fang.com/xf/' + yidicity + '/' + info.newCode + '.htm';
            } else {
                info.m_houseurl = 'http://m.fang.com/xf/' + encity + '/' + info.newCode + '.htm';
            }
